...
### Accepted aliases for `type`

CSVs exported from other platforms or generated by AI tools often use
different naming. The importer accepts these aliases and maps them to
the canonical Prisma enum:

| Aliases (case-insensitive, ignores spaces/underscores/hyphens) | Mapped to |
|---|---|
| `single_choice`, `single`, `single-choice`, `mcq`, `mcq_single`, `single select`, `single answer`, `radio` | `MCQ_SINGLE` |
| `multiple_choice`, `multi`, `multiple choice`, `mcq_multi`, `multi-select`, `multiple answer`, `multiple answers`, `checkbox` | `MCQ_MULTI` |
| `true_false`, `true/false`, `tf`, `boolean`, `bool`, `yes_no` | `TRUE_FALSE` |

### Accepted aliases for `difficulty`

| Aliases | Mapped to |
|---|---|
| `easy`, `beginner`, `basic`, `simple`, `low`, `junior`, `1` | `EASY` |
| `medium`, `intermediate`, `moderate`, `mid`, `mid-level`, `normal`, `average`, `2` | `MEDIUM` |
| `hard`, `advanced`, `difficult`, `expert`, `high`, `senior`, `3` | `HARD` |

### Accepted aliases for `correctAnswers`

| Format | Example | Notes |
|---|---|---|
| Letter | `A` or `C` | Canonical. Matches `optionA` / `optionC`. |
| Comma-separated letters | `"A,B,D"` | For `MCQ_MULTI`. Wrap in `"..."` so the inner comma doesn't split into columns. |
| 1-based index | `1` or `3` | Maps to `A`/`C`. Useful when exporting from systems that use indices. |
| Comma-separated indices | `"1,2,4"` | Same idea for `MCQ_MULTI`. |

Mixed formats are not allowed in the same cell — pick letters OR indices.
