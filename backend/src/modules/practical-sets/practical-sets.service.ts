...
export class PracticalSetsService {
  ...
  async assignSetToSession(sessionId: string, setId: string) {
    ...
    // Save the set on the session. If MCQ is done, also flip into practical phase
  ...
  async getAssignedSetForSession(sessionId: string) {
    const session = await this.prisma.examSession.findUnique({
      where: { id: sessionId },
      select: { practicalPaperSetId: true },
    });
    if (!session?.practicalPaperSetId) return null;
    const set = await this.getSet(session.practicalPaperSetId, { includeFiles: true, includeQuestions: true });
