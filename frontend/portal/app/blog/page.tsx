                ...
                <Link href={`/blog/${p.slug}`} style={{ textDecoration: 'none', display: 'block', height: '100%' }}>
                  <article className="web-card" style={{ padding: 0, overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                      ...
                      <h2 style={{ margin: '0 0 10px', fontSize: '18px', fontWeight: 700, color: 'var(--web-text)', lineHeight: 1.35, letterSpacing: '-0.01em' }}>{p.title}</h2>
                      {p.excerpt && <p style={{ margin: '0 0 16px', fontSize: '14px', color: 'var(--web-text-secondary)', lineHeight: 1.7, flex: 1 }}>{p.excerpt}</p>}
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto' }}>
                        <span style={{ fontSize: '12px', color: 'var(--web-text-muted)' }}>{p.authorName ? `${p.authorName} · ` : ''}{fmtDate(p.publishedAt)}</span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '13px', color: 'var(--web-gold)', fontWeight: 600 }}>Read <ArrowRight size={14} /></span>
                      </div>
                    </div>
                  </article>
                </Link>
