...
function SessionContent() {
  ...
  const startProctorCamera = useCallback(async () => {
    try {
      ...
      // Assign immediately if video element already mounted
  ...
  // WebRTC — must be called before any early returns (Rules of Hooks)
  ...
  if (!sessionId) return (
    <div style={{ padding: '40px', textAlign: 'center' }}>
      ...
      <Link href="/proctor/today" className="btn-primary" style={{ textDecoration: 'none', padding: '10px 24px' }}>View Today's Sessions â†’</Link>
    </div>
  )
  ...
  const candidateStream: MediaStream | null = remoteStreams.size > 0
    ? (Array.from(remoteStreams.values())[0] as MediaStream)
    : null

  // Build candidate tiles for MonitorGrid
  ...
  // MCQ results for PracticalPanel
  const mcqResults = [{
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '700', color: 'var(--text-primary)', margin: 0 }}>
            Live Session â€” {candidate?.firstName} {candidate?.lastName}
          </h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginTop: '4px' }}>
            {session.assessmentType?.name} Â· Phase: <strong style={{ color: 'var(--cyan)', textTransform: 'capitalize' }}>{phase}</strong>
          </p>
        </div>
      ...
      <div className="glass-card" style={{ padding: '12px 16px', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '14px' }}>
        ...
        <div>
          ...
          <p style={{ margin: '2px 0 0', fontSize: '12px', color: cameraActive ? 'var(--emerald)' : cameraError ? 'var(--rose)' : 'var(--text-muted)' }}>
            {cameraActive ? 'Active â€” candidates can see you' : cameraError ? 'Camera access denied' : 'Starting...'}
          </p>
      ...
      {/* â”€â”€ MCQ COMPLETE â†’ Assign Practical â”€â”€ */}
      {phase === 'mcq' && ['MCQ_COMPLETE', 'MCQ_SUBMITTED', 'AWAITING_PRACTICAL'].includes(session.status) && (
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px' }}>
          <PracticalPanel
            sessionId={sessionId}
            ...
            onPracticalStarted={() => { setPhase('practical'); qc.invalidateQueries({ queryKey: ['proctor-session', sessionId] }) }}
          />
          <FlagQueue flags={activeFlags} onFlagActioned={id => setResolvedFlagIds(p => [...p, id])} />
        </div>
      )}

      {/* â”€â”€ PHASE: PRACTICAL â”€â”€ */}
      {phase === 'practical' && (
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '20px' }}>
          <MonitorGrid
            sessionId={sessionId}
