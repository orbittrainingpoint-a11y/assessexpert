...
const VALUES: { icon: IconKey; title: string; desc: string; accent: string }[] = [
  { icon: 'target', title: 'Precision over Speed', desc: 'Every report is reviewed by a certified proctor before it reaches HR. We never sacrifice accuracy for turnaround time.', accent: '#C8A44E' },
  { icon: 'lock', title: 'Integrity First', desc: 'AI proctoring, facial recognition, and screen recording work together to ensure every result reflects genuine candidate ability.', accent: '#6366F1' },
...
const TEAM_ROLES = [
  ...
  { role: 'Master Proctor', desc: 'Senior operational lead — oversees all sessions, manages proctors, reviews escalated reports.', color: '#C8A44E' },
  { role: 'Exam Setup Master', desc: 'Content lead — builds and maintains the 500-question banks and practical task library.', color: '#6366F1' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--web-bg)', fontFamily: 'var(--web-sans)', overflowX: 'hidden' }}>
      ...
      <section style={{ padding: '60px 24px 0', maxWidth: '1100px', margin: '0 auto' }}>
        <Reveal>
          <div style={{ position: 'relative', padding: '56px 48px', background: 'rgba(200,164,78,0.03)', border: '1px solid rgba(200,164,78,0.12)', borderRadius: '20px', textAlign: 'center', overflow: 'hidden' }}>
            <div className="web-glow-orb" style={{ width: '300px', height: '300px', background: 'rgba(200,164,78,0.06)', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }} />
            <div style={{ position: 'relative' }}>
              <div style={{ fontSize: '48px', marginBottom: '20px', opacity: 0.4, color: 'var(--web-gold)', fontFamily: 'var(--web-serif)', lineHeight: 1 }}>&ldquo;</div>
              <p style={{ fontSize: '22px', fontWeight: 600, color: 'var(--web-text)', lineHeight: 1.65, margin: '0 auto 24px', maxWidth: '700px', fontFamily: 'var(--web-serif)' }}>
      ...
      <section style={{ padding: '100px 24px', maxWidth: '1280px', margin: '0 auto' }}>
        <Reveal style={{ textAlign: 'center', marginBottom: '56px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px,4vw,40px)', fontWeight: 800, color: 'var(--web-text)', margin: 0, letterSpacing: '-0.02em', fontFamily: 'var(--web-serif)' }}>What assessexpert Is &amp; Is Not</h2>
        </Reveal>
      ...
      <section style={{ background: 'rgba(255,255,255,0.015)', borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)', padding: '100px 24px' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center', marginBottom: '64px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px,4vw,40px)', fontWeight: 800, color: 'var(--web-text)', margin: 0, letterSpacing: '-0.02em', fontFamily: 'var(--web-serif)' }}>Our Values</h2>
          </Reveal>
      ...
      <section style={{ padding: '100px 24px', maxWidth: '1100px', margin: '0 auto' }}>
        <Reveal style={{ textAlign: 'center', marginBottom: '56px' }}>
          ...
          <h2 style={{ fontSize: 'clamp(28px,4vw,40px)', fontWeight: 800, color: 'var(--web-text)', margin: '0 0 12px', letterSpacing: '-0.02em', fontFamily: 'var(--web-serif)' }}>Our Team Structure</h2>
          <p style={{ color: 'var(--web-text-muted)', fontSize: '16px', margin: 0 }}>Every assessment is backed by a dedicated team of specialists.</p>
        </Reveal>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {TEAM_ROLES.map((t, i) => (
            <Reveal key={t.role} delay={i * 40}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '24px', padding: '24px 32px', background: 'rgba(255,255,255,0.025)', border: '1px solid var(--web-border)', borderRadius: '14px', flexWrap: 'wrap' }}>
                ...
                <p style={{ margin: 0, fontSize: '14px', color: 'var(--web-text-secondary)', lineHeight: 1.6, flex: 1 }}>{t.desc}</p>
              </div>
            </Reveal>
          ))}
        </div>
      ...
      <section style={{ background: 'rgba(255,255,255,0.015)', borderTop: '1px solid rgba(255,255,255,0.05)', padding: '100px 24px' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <Reveal style={{ textAlign: 'center', marginBottom: '64px' }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px,4vw,40px)', fontWeight: 800, color: 'var(--web-text)', margin: 0, letterSpacing: '-0.02em', fontFamily: 'var(--web-serif)' }}>Milestones</h2>
          </Reveal>
          <div style={{ position: 'relative', paddingLeft: '40px' }}>
            <div style={{ position: 'absolute', left: '15px', top: 0, bottom: 0, width: '1px', background: 'linear-gradient(180deg, var(--web-gold), rgba(200,164,78,0.1))' }} />
            {MILESTONES.map((m, i) => (
              <Reveal key={m.year} delay={i * 50} style={{ position: 'relative', marginBottom: i < MILESTONES.length - 1 ? '40px' : 0 }}>
                <div style={{ position: 'absolute', left: '-32px', top: '4px', width: '14px', height: '14px', background: 'var(--web-gold)', borderRadius: '50%', border: '3px solid var(--web-bg)', boxShadow: '0 0 12px rgba(200,164,78,0.4)' }} />
                <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
                  <span style={{ fontSize: '13px', fontWeight: 700, color: 'var(--web-gold)', minWidth: '48px', paddingTop: '2px' }}>{m.year}</span>
                  <p style={{ margin: 0, fontSize: '15px', color: 'var(--web-text-secondary)', lineHeight: 1.6 }}>{m.event}</p>
                </div>
              </Reveal>
            ))}
          </div>
      ...
      <section style={{ padding: '100px 24px', textAlign: 'center' }}>
        <Reveal style={{ maxWidth: '600px', margin: '0 auto' }}>
          <h2 style={{ fontSize: 'clamp(28px,4vw,40px)', fontWeight: 800, color: 'var(--web-text)', margin: '0 0 16px', letterSpacing: '-0.02em', fontFamily: 'var(--web-serif)' }}>Want to work with us?</h2>
          <p style={{ color: 'var(--web-text-secondary)', fontSize: '16px', marginBottom: '36px', lineHeight: 1.7 }}>All client relationships start with a conversation — no self-signup, no payment page.</p>
