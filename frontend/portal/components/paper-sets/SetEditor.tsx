...
export const BLANK_QUESTION: any = {
  prompt: '',
...
/**
 * Full editor for a single PracticalPaperSet — file library + ordered questions.
 * Shared between the dedicated /master-proctor/paper-sets page and the
 * "Practical" tab of /master-proctor/questions.
 */
export function SetEditor({ setId, onBack }: Props) {
  ...
  const [qForm, setQForm] = useState<any>(BLANK_QUESTION)
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '4px' }}>
            {set.assessmentType?.name} · {set.questions?.length || 0} questions · {set.files?.length || 0} files
            · <span className={`badge ${set.status === 'ACTIVE' ? 'badge-pass' : 'badge-pending'}`} style={{ fontSize: '10px', marginLeft: '4px' }}>{set.status}</span>
          </p>
        </div>
        {set.status !== 'ACTIVE' && (
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            ...
            <button type="button" className="btn-primary"
              onClick={() => { setEditingQuestion(null); setQForm(BLANK_QUESTION); setShowAddQ(true) }}
                ...
                <div key={q.id} className="glass-card" style={{ padding: '14px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      ...
                      <button type="button" className="btn-ghost" style={{ padding: '4px 10px', fontSize: '11px', color: 'var(--rose)', borderColor: 'rgba(225,29,72,0.3)' }}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {(set.files || []).map((f: any) => (
                <div key={f.id} className="glass-card" style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <button type="button" className="btn-ghost" style={{ padding: '3px 8px', fontSize: '10px', color: 'var(--rose)' }}
      ...
      {(showAddQ || editingQuestion) && (
        <QuestionModal
          ...
          onCancel={() => { setShowAddQ(false); setEditingQuestion(null); setQForm(BLANK_QUESTION) }}
          onSave={handleSave}
        />
      )}
    </div>
...
function QuestionModal({ form, setForm, setFiles, editing, saving, onCancel, onSave }: any) {
  ...
  const toggleFileRef = (fid: string) =>
    update({
      referencedFileIds: form.referencedFileIds.includes(fid)
